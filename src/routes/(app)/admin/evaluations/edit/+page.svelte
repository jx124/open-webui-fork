<script lang="ts">
	...
	import { user, evaluations, models } from '$lib/stores';
	import { onMount, getContext } from 'svelte';

	import { updateEvaluation, type EvaluationForm } from '$lib/apis/evaluations';
	...
	const submitHandler = async () => {
		loading = true;
		const evaluation = await updateEvaluation(localStorage.token, form_data).catch((error) => {
			toast.error(error);
			loading = false;
			return null;
		});
	...
	onMount(async () => {
		...
		const evaluation = $evaluations.filter((evaluation) => evaluation.id === form_data.id).at(0);
