...
export const userResetEmail = async (
	email: string,
) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/reset`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json'
		},
...
export const verifyUserOTP = async (
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/reset/verify`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json'
		},
...
export const getUserIDsByExcel = async (
	token: string,
	xlsx: ArrayBuffer
) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/ids/import`, {
		method: 'POST',
		headers: {
			'Content-Type': 'array',
			...(token && { authorization: `Bearer ${token}` })
