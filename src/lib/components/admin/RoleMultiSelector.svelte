<script lang="ts">
    ...
    let selectedItems = [];
    let unselectedItems = [];
    export let permittedRoles: number[];
	...
	$: filteredItems = searchValue
		? unselectedItems.filter((item) => item.label.toLowerCase().includes(searchValue.toLowerCase()))
		: unselectedItems;
    ...
    $: if (permittedRoles) {
        selectedItems = items.filter((item) => permittedRoles.includes(item.value));
        unselectedItems = items.filter((item) => !permittedRoles.includes(item.value));
    };
</script>
...
<div class="flex items-center w-full">
    ...
    >
        <div class="flex flex-wrap min-h-10 px-3 pt-2 w-full bg-transparent border dark:border-gray-600 outline-none rounded-lg">
            {#each selectedItems as role}
                <div
                    class="flex items-center text-nowrap w-fit gap-2 text-xs px-3 py-0.5 mr-2 mb-2 rounded-lg {`${buttonColors[getRoleColor(role.label)]}`}"
                >
                    <div
                        class="w-1 h-1 rounded-full {`${divColors[getRoleColor(role.label)]}`}"
                    />
                        {role.label}
                    <button
                        class="self-center"
                        type="button"
                        on:click={() => {
                            permittedRoles = permittedRoles.filter(
                                (selected) => selected !== role.value
                            );
                            // items = [ ...items, { label: role.name, value: role.id }]
        ...
        <DropdownMenu.Trigger aria-label={placeholder}>
            <button class="text-sm px-3 py-2 ml-2 transition rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-800"
                type="button"
            >
                <div class="self-center text-sm font-medium text-nowrap">Add Role</div>
            </button>
                ...
                <div class="px-3 my-2 max-h-64 overflow-y-auto scrollbar-hidden">
                    {#each filteredItems as item}
                        <button
                            ...
                            on:click={() => {
                                // selectedItems = [...selectedItems, { id: item.value, name: item.label } ]
                                // items = items.filter(
                                //     (selected) => selected.value !== item.value
                                // );
                                permittedRoles = [ ...permittedRoles, item.value ]
                            }}
                        >
                    ...
                    {:else}
                        <div class="block px-3 py-2 text-sm text-gray-700 dark:text-gray-100">
                            No results found, add more roles under Admin Panel > 
