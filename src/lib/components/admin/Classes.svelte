<script lang="ts">
	...
	import { deleteClassById, downloadChatsByClassId, getClassList } from '$lib/apis/classes';
	...
	{#each $classes.filter((c) => searchValue === '' || c.name
        ...
        >
            <div class=" flex flex-1 space-x-4 cursor-pointer w-full">
                <a href={`/admin/classes/edit?id=${encodeURIComponent(class_.id)}`}>
            ...
            <div class="flex flex-row space-x-1 self-center">
				<button
                    ...
                    type="button"
					on:click={() => downloadChatsByClassId(localStorage.token, class_.id)}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
                ...
                <a
                    class="self-center w-fit text-sm px-2 py-2 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
                    type="button"
                    href={`/admin/classes/edit?id=${encodeURIComponent(class_.id)}`}
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
